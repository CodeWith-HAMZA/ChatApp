...
const SignupCard = () => {
  const { setUser, User, showAlert }: object | null = useContext(
    AuthContext
  ) as object | null;
  const [Img, setImg] = useState<string>(
    "https://dummyimage.com/600x400/f5f5f5/e6e6e6.png&text=Not+Selected"
  );
  ...
  const Submit = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      console.log(User, "user", formObj.email, formObj.password);
      const userCredential = await await createUserWithEmailAndPassword(
        auth,
        formObj.email,
        formObj.password
      );
      console.log(userCredential, formObj, "eue");

      // * Setting/Preparing The Folder Structure Of The Image In DB, "User-Profile-Image" would be uploaded by its own "user-name"
      const storageRef = ref(
        storage,
        `User-Profile-Imgs/${formObj.username.replace(/\s/g, "_")}`
      );
      ...
      showAlert("green", "Success", "Successfully Registered");
    } catch (error: unknown) {
            ...
            <div className="flex justify-center items-center">
              <form className="shadow-lg p-4 rounded-md" onSubmit={Submit}>
                ...
                <div className="mb-6">
                  ...
                  >
                    <span className="rounded-md bg-blue-600 hover:bg-blue-700 text-white cursor-pointer border-2 p-2 px-4">
                      + Add Profile
                    </span>
                    <img
                      src={Img}
                      className="w-10 h-10 rounded-lg my-3"
                      alt=""
                    />
                  </label>
                </div>
