...
const ChatBar: React.FC<Props> = ({ SelectedUser }: Props): JSX.Element => {
  ...
  useEffect(() => {
    ...
    const unSubscribeForMessage = onSnapshot(
      ...
      query(collection(db, "_Chats"), where("combinedID", "==", COMBINED_ID_CharSum)),

      (snap: any) => {
        setMessages(snap?.docs)
        console.log(new Date(snap?.docs[0]?._document?.data?.value?.mapValue?.fields?.createdAt?.timestampValue))

        const firstDate = new Date(snap?.docs[0]?._document?.data?.value?.mapValue?.fields?.createdAt?.timestampValue)
        setMessages(snap?.docs.slice().sort((a, b) => new Date(a._document?.data?.value?.mapValue?.fields?.createdAt?.timestampValue) - new Date(b._document?.data?.value?.mapValue?.fields?.createdAt?.timestampValue)))
